...
def GetWeight(feature_name, weight_name, bias_name, train_labels):

    # load feature
    fr = open(feature_name, 'rb')
    feat = pickle.load(fr)
    fr.close()
    ...
    feature=feature.reshape(-1, 16, 5, 5)
    ...
    # save data
    fw = open(weight_name, 'wb')
    pickle.dump(weights, fw, protocol=2)
    fw.close()
    fw = open(bias_name, 'wb')
