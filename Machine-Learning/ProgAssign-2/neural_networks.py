...
class tanh:
    ...
    def forward(self, X):
        ...
        forward_output = (np.exp(X)-np.exp(-X))/(np.exp(X)+np.exp(-X))
        return forward_output
    ...
    def backward(self, X, grad):
        ...
        backward_output = grad * (1 - tanh().forward(X)**2)
        return backward_output
    ...
    for module_name, module in model.items():
        ...
        if hasattr(module, 'params'):
            for key, _ in module.params.items():
                #print('----',key, module.gradient[key].shape,  module.params[key].shape)
                ...
                if _alpha > 0.0:
                    ...
                    #################################################################################
                    #print(module.gradient[key].shape, g.shape)
